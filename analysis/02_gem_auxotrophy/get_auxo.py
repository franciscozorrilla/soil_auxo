"""Predict amino-acid (and other metabolite) auxotrophies from the genome-scale metabolic
models in data/models/.

For each model this writes a per-model `<model>.xml.aux` file holding the ReFramed
...
for file in glob.iglob(r'data/models/*.xml'):
	...
	try:
		...
		n = text_file.write(str(auxotrophy.auxotrophies(model)))
