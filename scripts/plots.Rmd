...
# Set working directory one folder outside /final_figs_data
 setwd("~/path/to/workdir/containing/final/figs/data/folder")
...
read.delim("final_figs_data/auxo_df.tsv") -> auxo_df
...
# load metadata
metadata = read.delim("final_figs_data/soil_auxo_metadata_full.tsv")
...
bigg_mets = read.delim("final_figs_data/bigg_classes.tsv")
...
auxolab = read.delim("final_figs_data/auxolab_summary.tsv") %>% 
  replace(is.na(.), 0) %>% 
...
auxopred2 = read.delim("final_figs_data/auxopred_default.tsv") %>% 
...
write_tsv(auxo2 %>% mutate(auxo_lab=as.integer(auxo_lab)-1,auxo_model=as.integer(auxo_model)-1) %>% 
              ungroup() %>% group_by(genome) %>% summarise(auxo_lab_sum=sum(auxo_lab),auxo_model_sum=sum(auxo_model)),file = "~/Documents/soil_auxo/final_figs_data/figure3panelA.tsv")
...
contigs_bp = read.delim("final_figs_data/genome_contigs_bp.tsv")

# Load checkm data with completeness and contam
checkm = read.delim("final_figs_data/checkm_summary.tsv") %>% 
...
# Load kemet data
kemet = read.delim("final_figs_data/kemet_summary.tsv",header = FALSE) %>% select(-V8) %>% 
...
write_tsv(kemet_auxo_all %>% mutate(auxo_lab=as.integer(auxo_lab)-1,auxo_kemet=as.integer(auxo_kemet)-1) %>% 
            ...
            left_join(.,auxo2 %>% mutate(auxo_lab=as.integer(auxo_lab)-1,auxo_model=as.integer(auxo_model)-1) %>% 
              ungroup() %>% group_by(genome) %>% summarise(auxo_lab_sum=sum(auxo_lab),auxo_model_sum=sum(auxo_model)) %>%
                select(genome,auxo_lab_sum)) %>% drop_na(), file = "~/Documents/soil_auxo/final_figs_data/figure3panelB.tsv")
...
# write file for leo
write_tsv(x = genes_count %>% mutate(AAs_auxo=as.factor(AAs_auxo)) %>% mutate(AAs_auxo=ifelse(is.na(AAs_auxo),"Not assayed",ifelse(AAs_auxo==1,"Auxotroph","Prototroph"))) %>% mutate(AAs_auxo=factor(AAs_auxo,levels=c("Not assayed","Prototroph","Auxotroph"))),file="~/Documents/soil_auxo/final_figs_data/figure3panelC.tsv")
...
# Load IS finder hits
isfinder_hits = read.delim("final_figs_data/ISfinder_k1000.tsv") %>%
...
# write file for leo
write_tsv(x=isfinder_count %>% mutate(AAs_auxo=ifelse(is.na(AAs_auxo),"Not assayed",ifelse(AAs_auxo==1,"Auxotroph","Prototroph")),AAs_auxo=factor(AAs_auxo,levels=c("Not assayed","Prototroph","Auxotroph")),count_norm=1000000*count_norm),file = "~/Documents/soil_auxo/final_figs_data/figure3panelD.tsv")
...
genomad_plasmid_genes = read.delim("final_figs_data/cat_mags_plasmid_genes.tsv")
#genomad plasmids
genomad_plasmid = read.delim("final_figs_data/cat_mags_plasmid_summary.tsv") %>% mutate(Sample_ID=gsub("_.*$","",seq_name),genome=gsub("^([^_]+_[^_]+).*", "\\1",seq_name)) %>% left_join(.,metadata)
...
genomad_virus_genes = read.delim("final_figs_data/cat_mags_virus_genes.tsv")
#genomad virus
genomad_virus = read.delim("final_figs_data/cat_mags_virus_summary.tsv") %>% mutate(Sample_ID=gsub("_.*$","",seq_name),genome=gsub("^([^_]+_[^_]+).*", "\\1",seq_name)) %>% left_join(.,metadata)
...
# load tree file
tree=read.tree("final_figs_data/protein.tre.treefile")
